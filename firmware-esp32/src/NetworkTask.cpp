...
void connectWiFiManager() {
    Serial.println("[NetworkTask] Khởi động WiFiManager (LUÔN MỞ AP)...");

    WiFiManager wm;

    // (Tuỳ chọn) reset WiFi cũ để tránh lỗi
    wm.resetSettings();
...
// ── Lưu trạng thái mode hiện tại để xử lý state correctly ──
static String s_CurrentModeFromFirebase = "idle";
...
void firebaseCallback(StreamData data) {
   if (data.eventType() == "put" || data.eventType() == "patch") {
        ...
        // Lệnh Thức ăn
            ...
            s_CurrentModeFromFirebase = mode;  // Lưu mode hiện tại
            ...
            if (mode == "auto") { 
                ...
                // KHÔNG gửi command ngay, chờ target_gram
                // Vì GRAM sẽ được trigger bởi target_gram value
                Serial.printf("[Stream] Mode thay đổi thành GRAM, chờ target_gram...\n");
                return;  // Không gửi command
            }
            else if (mode == "manual") { 
        ...
        // FIX: Xử lý state dựa trên mode hiện tại
        else if (path.indexOf("state") != -1 && path.indexOf("thucan") != -1) {
            bool stateValue = data.boolData();
            CommandData_t cmd = {.timestamp = millis()};
            
            // Dùng mode đã lưu để xử lý state đúng cách
            if (s_CurrentModeFromFirebase == "auto") {
                cmd.type = CMD_THUCAN_AUTO;
                cmd.value = stateValue ? 1.0f : 0.0f;
                Serial.printf("[Stream] CMD_THUCAN_STATE: AUTO %s\n", stateValue ? "ON" : "OFF");
                xQueueSend(xQueue_FeedCommands, &cmd, 0);
            }
            else if (s_CurrentModeFromFirebase == "gram") {
                // CHỈ XỬ LÝ GRAM qua target_gram field, KHÔNG qua state
                // state sẽ có giá trị kèm, nhưng target_gram=0 khi stop, target_gram=X khi start
                // Nên không cần gửi command từ state change
                Serial.printf("[Stream] State change cho GRAM mode, bỏ qua (sẽ dùng target_gram)\n");
                // Không gửi command - chỉ dùng target_gram
                return;
            }
            else if (s_CurrentModeFromFirebase == "manual") {
                cmd.type = CMD_THUCAN_MANUAL;
                cmd.value = stateValue ? 1.0f : 0.0f;
                Serial.printf("[Stream] CMD_THUCAN_STATE: MANUAL %s\n", stateValue ? "ON" : "OFF");
                xQueueSend(xQueue_FeedCommands, &cmd, 0);
            }
            else {
                // mode không xác định
                Serial.printf("[Stream] Mode không xác định: %s, bỏ qua state change\n", s_CurrentModeFromFirebase.c_str());
                return;
            }
        }
    }
