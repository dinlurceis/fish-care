...
void sensorTaskLoop(void* unused) {
    ...
    for (;;) {
        ...
        // 1. PEEK ĐỂ LẤY DATA HIỆN TẠI TỪ QUEUE
        // Việc này giúp bảo toàn thông số 'weight'.
        if (xQueue_SensorData != nullptr) {
            xQueuePeek(xQueue_SensorData, &reading, 0);
        }
        
        // 2. CẬP NHẬT CÁC THÔNG SỐ MÔI TRƯỜNG 
...
bool SensorTask_IsDS18B20Connected() {
    // Kiểm tra xem wrapper DS18B20 có hợp lệ không (đã được khởi tạo)
